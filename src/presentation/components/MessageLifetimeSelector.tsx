...
import { useMessageLifetimes } from '@/presentation/hooks/useMessageLifetimes';
import { MessageLifetime } from '@/core/domain/entities/MessageLifetime';
...
export function MessageLifetimeSelector({ 
  ...
  const { lifetimes, isLoading, error } = useMessageLifetimes();
  const [customHoursInput, setCustomHoursInput] = useState(customHours || '');

  // Si hay una duración preseleccionada, usarla
  useEffect(() => {
    if (selectedLifetime && lifetimes.length > 0) {
      const selected = lifetimes.find(l => l.name === selectedLifetime);
      if (selected) {
        onLifetimeChange(selected);
      }
    }
  ...
  const handleCustomHoursChange = (hours: string) => {
    ...
    if (hoursNum > 0 && hoursNum <= 168) { // Máximo 7 días
    ...
    <div className="flex items-center gap-2">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          disabled={disabled || !selectedLifetime?.isTemporary}
          className="px-3 py-2 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
        />
      ...
      {selectedLifetime && (
        <div className="px-3 py-2 bg-gray-50 rounded-lg border border-gray-200">
          <div className="text-xs text-gray-600">
            Tiempo restante: {selectedLifetime.hours}h
          </div>
        </div>
      )}
    </div>
