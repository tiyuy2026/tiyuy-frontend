...
// Importar axiosClient centralizado
...
export function useUserChats(filter: 'all' | 'unread' | 'favorites' = 'all', page: number = 0, size: number = 20) {
  return useQuery({
    ...
    queryFn: async (): Promise<{ content: Chat[]; totalPages: number; totalElements: number }> => {
      const response = await axiosClient.get(`/contacts/extended/chats?filter=${filter}&page=${page}&size=${size}`);
      if (!response.data) {
...
export function useChatMessages(chatId: number, page: number = 0, size: number = 20) {
  return useQuery({
    ...
    queryFn: async (): Promise<{ content: ChatMessage[]; totalPages: number; totalElements: number }> => {
      const response = await axiosClient.get(`/contacts/extended/chats/${chatId}/messages?page=${page}&size=${size}`);
      if (!response.data) {
...
export function useStatusPosts(page: number = 0, size: number = 10) {
  return useQuery({
    ...
    queryFn: async (): Promise<{ content: StatusPost[]; totalPages: number; totalElements: number }> => {
      const response = await axiosClient.get(`/contacts/extended/status?page=${page}&size=${size}`);
      if (!response.data) {
...
export function useSendMessage() {
  ...
  return useMutation({
    mutationFn: async ({ chatId, content, type, mediaUrl }: {
      ...
      const response = await axiosClient.post(`/contacts/extended/chats/${chatId}/messages`, { content, type, mediaUrl });
      return response.data;
    },
    onSuccess: () => {
...
export function useCreateStatus() {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      const response = await axiosClient.post('/contacts/extended/status', data);
      return response.data;
    },
    onSuccess: () => {
...
export function useMarkAsRead() {
  ...
  return useMutation({
    mutationFn: async (chatId: number) => {
      const response = await axiosClient.put(`/contacts/extended/chats/${chatId}/read`);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['user-chats'] });
