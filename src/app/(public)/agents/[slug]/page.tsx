...
import { MapPin, Home, ArrowLeft, ChevronRight, Mail, Phone, Send, Loader2, CheckCircle2, MessageCircle, AlertCircle, User, Star, LogIn, UserPlus, X } from 'lucide-react';
import { publicApiClient, apiClient } from '@/infrastructure/api/axios-client';
import { useAuthStore } from '@/presentation/store/authStore';

interface AgentDetail {
  id: number; slug: string; name: string; firstName: string; lastName: string;
  avatarUrl: string | null; email: string; phone: string; city: string; description: string;
  totalPropertiesCount: number; activePropertiesCount: number; inactivePropertiesCount: number; propertiesUrl: string;
}
...
export default function AgentDetailPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await publicApiClient.post('/interactions/leads', {
        ...
        message: formData.message.trim(), toAgent: agent?.name, toAgentId: agent?.id, source: 'agente_perfil', recipientUserId: agent?.id,
      });
      setFormSent(true);
  ...
  const handleRate = async (score: number) => {
    ...
    try {
      await apiClient.post('/interactions/ratings', { ratedUserId: agent.id, score, comment: '' });
      setUserRating(score); setRatingMsg('¡Calificación guardada!');
      const r = await publicApiClient.get(`/interactions/ratings/user/${agent.id}/summary`);
      setRatings(r.data);
    } catch (err: any) { setRatingMsg(err?.response?.data?.message || 'Error'); } finally { setSendingRating(false); }
  };
  ...
  const waNumber = agent?.phone?.replace(/[^0-9]/g, '') || '';
  const waMessage = encodeURIComponent(`Hola ${agent?.name || ''}, vi tu perfil en Tiyuy.`);
  ...
  if (!agent) return (
    <div className="min-h-screen bg-[#faf9f7] flex items-center justify-center">
      <div className="text-center"><User className="w-12 h-12 text-[#d4d0ca] mx-auto mb-3" /><p className="text-sm font-medium text-[#a0988c]">Agente no encontrado</p><Link href="/agents" className="text-sm text-[#2d5a3d] hover:underline mt-2 inline-block">Volver al directorio</Link></div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#faf9f7]">
      <div className="relative bg-gradient-to-br from-[#3f9800]/90 via-[#3f9800]/85 to-[#2d6e00]/90 text-white overflow-hidden">
        <div className="absolute inset-0 z-0" style={{ backgroundImage: `url('https://edifica.com.pe/blog/wp-content/uploads/espacios-relajacion.jpg')`, backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.15 }} />
        <div className="relative z-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <Link href="/agents" className="inline-flex items-center gap-1 text-sm text-[#cbd5c6] hover:text-white mb-6"><ArrowLeft className="w-4 h-4" /> Volver al directorio</Link>
            <div className="flex items-start gap-6">
              {agent.avatarUrl ? <img src={agent.avatarUrl} alt={agent.name} className="w-20 h-20 rounded-2xl object-cover flex-shrink-0 border border-white/10" />
              : <div className="w-20 h-20 rounded-2xl bg-[#f5f1eb] flex items-center justify-center text-[#2d5a3d] font-bold text-3xl flex-shrink-0">{agent.firstName?.[0] || 'A'}</div>}
              <div className="flex-1"><h1 className="text-3xl sm:text-4xl font-bold">{agent.name}</h1><div className="flex flex-wrap items-center gap-4 mt-2 text-[#cbd5c6] text-sm">{agent.city && <span className="flex items-center gap-1"><MapPin className="w-4 h-4" />{agent.city}</span>}</div></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <Link href={agent.propertiesUrl || '#'} className="bg-white rounded-2xl border-2 border-[#e8e3dd] p-5 hover:shadow-[0_12px_40px_rgba(100,204,57,0.08)] hover:border-[#64cc39] transition-all">
            <div className="flex items-center gap-3 mb-2"><div className="w-10 h-10 rounded-xl bg-[#f5f1eb] flex items-center justify-center text-[#2d5a3d]"><Home className="w-5 h-5" /></div><div><p className="text-2xl font-bold text-[#2d2a24]">{agent.activePropertiesCount}</p><p className="text-xs text-[#a0988c]">Propiedades activas</p></div></div>
            {agent.activePropertiesCount > 0 && <span className="text-xs font-medium text-[#2d5a3d] flex items-center gap-1 mt-2">Ver propiedades <ChevronRight className="w-3 h-3" /></span>}
          </Link>
          <div className="bg-white rounded-2xl border-2 border-[#e8e3dd] p-5">
            <div className="flex items-center gap-3 mb-2"><div className="w-10 h-10 rounded-xl bg-[#f0ece6] flex items-center justify-center text-[#8a8275]"><Home className="w-5 h-5" /></div><div><p className="text-2xl font-bold text-[#2d2a24]">{agent.totalPropertiesCount}</p><p className="text-xs text-[#a0988c]">Total publicadas</p></div></div>
          </div>
          <div className="bg-white rounded-2xl border-2 border-[#e8e3dd] p-5">
            <div className="flex items-center gap-3 mb-2"><div className="w-10 h-10 rounded-xl bg-[#fef3e7] flex items-center justify-center text-[#d97706]"><Home className="w-5 h-5" /></div><div><p className="text-2xl font-bold text-[#2d2a24]">{agent.inactivePropertiesCount}</p><p className="text-xs text-[#a0988c]">Vendidas / Inactivas</p></div></div>
          </div>
        </div>

        {agent.description && <div className="bg-white rounded-3xl border border-[#e8e3dd] p-7"><h2 className="text-lg font-semibold text-[#2d2a24] mb-3">Acerca de</h2><p className="text-sm text-[#8a8275] leading-relaxed">{agent.description}</p></div>}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white rounded-3xl border border-[#e8e3dd] p-7">
            ...
            <div className="space-y-3 mb-6">
              {agent.email && <a href={`mailto:${agent.email}`} className="flex items-center gap-3 text-sm text-[#8a8275] hover:text-[#2d5a3d]"><Mail className="w-4 h-4 text-[#a0988c]" />{agent.email}</a>}
              {agent.phone && <a href={`tel:${agent.phone}`} className="flex items-center gap-3 text-sm text-[#8a8275] hover:text-[#2d5a3d]"><Phone className="w-4 h-4 text-[#a0988c]" />{agent.phone}</a>}
              {agent.city && <div className="flex items-center gap-3 text-sm text-[#8a8275]"><MapPin className="w-4 h-4 text-[#a0988c]" />{agent.city}</div>}
            </div>
            ...
            {showForm && !formSent && (
              <form onSubmit={handleSubmit} className="border-t border-[#f0ece6] pt-5 mt-4 space-y-4">
                ...
                <div><label className="block text-xs font-medium">Mensaje *</label><textarea value={formData.message} onChange={e => setFormData(p => ({...p, message: e.target.value}))} rows={3} className="w-full px-4 py-2.5 border border-[#e8e3dd] rounded-xl text-sm outline-none bg-[#faf9f7] resize-none" placeholder={`Hola ${agent.name}...`} /></div>
          ...
          <div className="bg-white rounded-3xl border border-[#e8e3dd] p-7">
            <h2 className="text-lg font-semibold text-[#2d2a24] mb-4">Calificar agente</h2>
            ...
            <div className="border-t border-[#f0ece6] pt-5">
              ...
              {userRating === 0 && !ratingMsg && <p className="text-xs text-[#a0988c]">Debes iniciar sesión para calificar</p>}
            </div>
          </div>
        </div>
      </div>
      ...
      {showAuthModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setShowAuthModal(false)}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 relative" onClick={e => e.stopPropagation()}>
            ...
            <h3 className="text-lg font-bold text-gray-900 text-center mb-2">Calificar agente</h3>
