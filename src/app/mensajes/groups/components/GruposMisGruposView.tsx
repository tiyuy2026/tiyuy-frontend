            ...
            <div className="p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {misGrupos.map((group: any) => (
                        <div key={group.id} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden hover:shadow-md transition-all cursor-pointer">
                            <div className="h-28 bg-gradient-to-br from-green-500 to-green-700 flex items-center justify-center" onClick={() => onGroupSelect(group)}>
                                <EntityIcon name={group.name} className="w-14 h-14 text-brand" />
                            </div>
                            <div className="p-3" onClick={() => onGroupSelect(group)}>
                                <h3 className="font-bold text-gray-900 dark:text-gray-100 text-sm line-clamp-2 leading-snug mb-1">{group.name}</h3>
                                <p className="text-xs text-gray-500 dark:text-gray-400">{group.memberCount || 0} miembros  {group.postCount || 0} publicaciones</p>
                            </div>
                            <div className="px-3 pb-3 flex items-center gap-2">
                                <button onClick={() => onGroupSelect(group)} className="flex-1 py-1.5 bg-green-50 text-green-700 text-xs font-semibold rounded-lg hover:bg-green-500 hover:text-white transition-colors">Ver grupo</button>
                                <button onClick={(e) => { e.stopPropagation(); setShareTarget({ title: group.name, link: `${window.location.origin}/groups/${group.id}` }); }} className="w-8 h-8 flex items-center justify-center bg-gray-100 dark:bg-gray-700 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
